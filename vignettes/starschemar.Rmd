...
## Star schema definition and transformation
...
We are going to modify the attribute names of the *when* dimension for the two design cases. Data according to age:
...
Reviewing the dimensions we can detect the need for updates.
...
In the case of the who dimension, we want to include a code in each field so that they are sorted increasingly based on their value.
...
In each case, we substitute the old value for the new one in the indicated field.
...
We can work both at the star or constellation level. In this example, we are going to do it at the constellation level, as we have been doing. 
...
The *Boston* data has disappeared and the surrogate keys have been reassigned.
...
1. Finally, basic queries can be performed from R  (*query functions*) also including geographic data in the result (*geographic definition and query functions*).
...
There are many multidimensional query tools available. The exported data, once stored in files, can be used directly from them. You can also perform basic queries from R on data in the `multistar` format, using the following functions:
